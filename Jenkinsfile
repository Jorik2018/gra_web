...
javac ^
  --release 8 ^
  -cp "war_tmp\\WEB-INF\\lib\\*;lib\\*;%WILDFLY_HOME%\\modules\\system\\layers\\base\\javax\\json\\api\\main\\jakarta.json-api-1.1.6.jar;%WILDFLY_HOME%\\modules\\system\\layers\\base\\javax\\enterprise\\api\\main\\*;%WILDFLY_HOME%\\modules\\system\\layers\\base\\javax\\faces\\api\\main\\*" ^
  -sourcepath src ^
  -d build\\classes ^
  @build\\sources.txt
  ...
  --patch "%TARGET_ISOBIT_JAR%=org.isobit.app.jsf.UserController"
        ...
        stage('VERIFY PATCHED WAR') {
            ...
            steps {
                bat '''
                    ...
                    if not exist "verify_tmp\\WEB-INF\\lib\\%TARGET_ISOBIT_JAR%" (
                        echo ERROR: No existe %TARGET_ISOBIT_JAR%
                        exit /b 1
                    )

                    jar -tf "verify_tmp\\WEB-INF\\lib\\%TARGET_ISOBIT_JAR%" ^
                      | findstr /I /C:"org/isobit/app/jsf/UserController.class"

                    if errorlevel 1 (
                        echo ERROR: UserController.class no encontrado
                        exit /b 1
                    )
    ...
    post {
        ...
        success {
            ...
            echo 'UserController parcheado quirurgicamente.'
